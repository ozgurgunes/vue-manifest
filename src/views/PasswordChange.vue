  ...
  <b-col id="login-view" offset="4" cols="4">
    ...
    <b-form @submit="submitForm()" v-on:submit.prevent>
      ...
      <b-form-group label="Old Password"
        ...
        <b-input type="password" id="old_password" placeholder="Enter a password"
          v-model="form.oldPassword" :class="{ 'is-invalid': errors.oldPassword }" />
      </b-form-group>

      <b-form-group label="New Password"
        :invalid-feedback="fieldError(errors.newPassword1)">
        <b-input type="password" id="new_password1" placeholder="Enter a password"
          v-model="form.newPassword1" :class="{ 'is-invalid': errors.newPassword1 }" />
      </b-form-group>

      <b-form-group label="Confrim password"
        :invalid-feedback="fieldError(errors.newPassword2)">
        <b-input type="password" id="new_password2" placeholder="Enter password again"
          v-model="form.newPassword2" :class="{ 'is-invalid': errors.newPassword2 }" />
      </b-form-group>
...
export default {
  ...
  methods: {
    ...
    submitForm () {
      this.$store.dispatch('reactor/PASSWORD_CHANGE', this.form)
        .then(() => {
